...
export async function GET() {
  try {
    ...
    if (!res.ok) return NextResponse.json(emptyForecast("DWD ICON", `upstream ${res.status}`), { status: 502 });
    ...
    return NextResponse.json(emptyForecast("DWD ICON", e.message), { status: 503 });
  }
}
